### Functions from https://github.com/NoemieJaquier/GaBOtorch/blob/master/BoManifolds/Riemannian_utils/spd_utils_torch.py
import numpy as np
import torch
'''
...
def affine_invariant_distance_torch(x1, x2, diagonal_distance=False):
    ...
    #TODO: Later functions require double, recheck to fix
    x1 = x1.double()
    x2 = x2.double()
    ...
    try:
        ...
        # TODO: Cholesky sometimes fails even for full rank matrices
        #       Adding some regularization noise to re-condition for now
        reg = torch.eye(dim)
        reg = reg.unsqueeze(0).unsqueeze(-3).repeat(x1.shape[0], x1.shape[1], 1, 1)
        x1_chol = torch.cholesky(x1 + 0.1*reg)
    ...
    out = torch.sqrt(sumlogeigv2 + 1e-15).double()
