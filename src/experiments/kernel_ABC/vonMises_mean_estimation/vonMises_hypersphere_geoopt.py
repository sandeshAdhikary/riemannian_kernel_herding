...
# dims = 3
dims = 6
loc = torch.zeros(dims).reshape(1, -1)
loc[:, 0] = 1.
loc = loc / torch.norm(loc)  # Normalized so its on the sphere
true_loc = loc
conc = torch.tensor([5.0]).reshape(1, -1)  # Defines concentration around loc
...
for seed in seeds:
    ...
    y = dist.sample(num_data).reshape(-1,dims)
        # .numpy().reshape(-1, dims)
    ...
    for exp, reg, bandwidth_scale in zip(exps, regs, bandwidth_scales):
        ...
        kernel_theta = Laplacian(bandwidth=bandwidth_scale, manifold="Sphere")
        ...
        errs_over_time = torch.norm(torch.from_numpy(theta_estimates) - true_loc, dim=(1))
