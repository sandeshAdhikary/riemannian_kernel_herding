...
from src.manifold_utils.spd_utils import isPD as onManifold
import geoopt
import torch
from src.distributions.wishart import InverseWishart
torch.set_default_dtype(torch.double)

def sampler(dist_type, dims, num_samples):
    if dist_type == "uniform":
        manifold = geoopt.SymmetricPositiveDefinite()
        return manifold.random((num_samples, dims, dims)).detach().numpy()
        # return special_ortho_group.rvs(dims, size=num_samples)
    elif dist_type == "inverse-wishart":
        nu = dims + 2  # must be larger than dims
        K = 1.0 * torch.diag(torch.rand(dims))
        dist = InverseWishart(nu, K)
        return dist.sample(num_samples).numpy().squeeze()
    else:
...
for bandwidth in tqdm(bandwidths):
    ...
    for trial in np.arange(num_trials):
        ...
        data_x = sampler(dist_type, dims, num_samples)
...
plt.tight_layout()
plt.savefig('kernel_pd_test/kernel_pd_test_{}_{}'.format(manifold_type, dist_type))
